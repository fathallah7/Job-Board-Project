            ...
            <div class="grid sm:grid-cols-1 md:grid-cols-2  gap-6 bg-white p-10 rounded-lg shadow-lg">
                ...
                <div>
                    ...
                    <select name="company_id"
                        ...
                        @foreach ($companies as $company)
                            <option {{ old('company_id') == $company->id ? 'selected' : '' }} value="{{ $company->id }}">
                                {{ $company->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="" rows="3"
                        class=" w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"></textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
