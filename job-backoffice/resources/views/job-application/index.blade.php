            ...
            @forelse ($applications as $application)
                <tr class="border-b border-gray-200 hover:bg-gray-100 transition duration">
                    <td class="py-3 px-4 font-medium">
                        ...
                        @else
                            <a href="{{ route('job-applications.show', $application->id) }}" class="text-blue-700 hover:underline">
                    ...
                    @if ($application->status == 'accepted')
                        <td class="py-3 px-4 text-green-700 font-bold">{{ $application->status }}</td>
                    @elseif ($application->status == 'pending')
                        <td class="py-3 px-4 text-yellow-600 font-bold">{{ $application->status }}</td>
                    @else
                        <td class="py-3 px-4 text-red-700 font-bold">{{ $application->status }}</td>
                    @endif
    ...
    <div class="md:hidden space-y-4 p-4">
        @forelse ($applications as $application)
            <div class="border rounded-lg p-4 shadow-sm bg-gray-50">
                ...
                <div class="mb-2">
                    <strong>Job Vacancy:</strong> {{ $application->JobVacancy->title ?? "N/A" }}
                </div>
                <div class="mb-2">
                    <strong>Company:</strong> {{ $application->JobVacancy->Company->name ?? "N/A" }}
                </div>
                <div class="mb-2">
                    <strong>Status:</strong>
                    <span class="
                        font-bold
                        {{ $application->status == 'accepted' ? 'text-green-700' : ($application->status == 'pending' ? 'text-yellow-600' : 'text-red-700') }}
                    ">
                        {{ $application->status }}
                    </span>
                </div>
