<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Company Information')}}  ( {{$company->name}} )
        </h2>
    </x-slot>
                    ...
                    <label for="industry" class="block mb-2 text-sm font-medium text-gray-700">Industry</label>
                        <select name="industry" class="block w-full rounded-md border border-slate-300 px-4 py-3 text-sm shadow-sm focus:border-sky-500 focus:ring-sky-500 focus:outline-none">
                            @foreach ($industries as $industry)
                                <option value="{{ old('industry' , $company->industry) }}">{{$industry}}</option>
                            @endforeach
                        </select>
                </div>
                ...
                {{-- Owner Email --}}
                    <div class="mb-4">
                        ...
                        <input disabled type="email" name="owner_email" id="owner_email" value="{{ old('owner_email' , $company->owner->email) }}"
                            class="bg-gray-100 mt-1 block w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    </div>
